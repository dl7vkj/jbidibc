...
public class SysClockMessage extends BidibMessage {
    public SysClockMessage(Calendar date, int factor) {
        super(0, BidibLibrary.MSG_SYS_CLOCK, getMinute(date), getHour(date), getDay(date),
                getAccelerationFactor(factor));
    }
    ...
    public static byte getDay(Calendar date) {
        return (byte) ((((date.get(Calendar.DAY_OF_WEEK) + 5) % 7) | 0x40) & 0x47);
    }

    public static byte getHour(Calendar date) {
        return (byte) ((date.get(Calendar.HOUR_OF_DAY) | 0x80) & 0x9F);
    }
